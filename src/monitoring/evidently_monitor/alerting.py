...
from .prometheus_metric import (
    ...
    MODEL_ACCURACY_LIVE, MODEL_ACCURACY_REFERENCE, ALERT_COUNT
)
...
class AlertEngine:
    ...
    def process_reports(self, reports, date_str):
        ...
        if classy_report:
            ...
            try:
                MODEL_ACCURACY_LIVE.set(acc_curr if acc_curr is not None else 0)
                MODEL_ACCURACY_REFERENCE.set(acc_ref if acc_ref is not None else 0)
            except Exception:
                pass
